...
TBinaryTree BST_remove_duplicates(TBinaryTree bst) {
    ...
    bst->left =  BST_remove_duplicates(bst->left);
    
    bst->left = binarytree_delete(bst->left,bst->info);
    bst->right = binarytree_delete(bst->right,bst->info);
    
    bst->right =  BST_remove_duplicates(bst->right);
