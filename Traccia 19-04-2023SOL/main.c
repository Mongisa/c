...
TInfo1* bst_partial_search(TBST tree, TInfo1 info, int max) {
    ...
    if (max <= 0 || tree == NULL) return NULL;
...
void hashTable_delete(THashTable* ht, TKey key) {
    ...
    unsigned int bucket_index = key_hash(key) % ht->n_bucket;
    
    TInfo2 t;
    
    strcpy(t.key,key);
    
    ht->bucket[bucket_index] = list_delete(ht->bucket[bucket_index],t);
...
void hashTable_partial_print(THashTable* ht, TBST t, int max) {
    ...
    for (int i = 0; i < ht->n_bucket; i++)
        for (THLNode *node = ht->bucket[i]; node != NULL; node = node->link)
            if(!bst_partial_search(t,node->info.key,max)) {
...
int main(int argc, char** argv) {
    ...
    //hashTable_insert(SD2, "quaranta", 40);
